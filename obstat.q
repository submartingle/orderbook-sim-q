...
OBstats:{[res]
	...
	avgTsize:first first value flip select avg Size from trades; /not the same as liquidity on the orderbook
	Tnum:count trades;
	...
	/select avgSpread: avg(askprice_1 - bidprice_1) by 1 xbar time.minute from res;
	...
	/select imbRatio: (sum(bidsize_1+bidsize_2+bidsize_3+bidsize_4+bidsize_5) - sum(asksize_1+asksize_2+asksize_3+asksize_4+asksize_5))%sum(bidsize_1+bidsize_2+bidsize_3+bidsize_4+bidsize_5+asksize_1+asksize_2+asksize_3+asksize_4+asksize_5) by 1 xbar time.minute from res;
	...
	maximb:value max select imbRatio: (sum(Tbidsizes - Tasksizes))%sum(Tvol) by 10 xbar time.second from res;
	minimb:value min select imbRatio: (sum(Tbidsizes - Tasksizes))%sum(Tvol) by 10 xbar time.second from res;
	/The reduction in variance from snapshots to aggregated windows indicates liquidity imbalances are transient.
    :`num_trades`avgTradeSize`ExCostWMid_type4`avgTradeSpread_1m`imbalanceRatio_10s!(Tnum;avgTsize;(rnd[mincst;0.001],rnd[maxcst;0.001]);(minSpr,maxSpr);(rnd[minimb;0.001],rnd[maximb;0.001]))}
